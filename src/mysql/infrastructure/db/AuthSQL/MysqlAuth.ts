...
export default class MysqlAuthRepository implements AuthRepoInterface {
  ...
  public async register(usuario: MysqlUsuario): Promise<boolean> {
    try {
      ...
      // Debug
      console.log("Usuario creado con CI:", usuario.ci);

      const res = await this.createCarrito(usuario.ci);
      if (!res) {
        console.error("Error al crear el carrito para el usuario:", usuario.ci);
      }
  ...
  async createCarrito(token: string): Promise<boolean> {
    try {
      console.log("Intentando crear carrito para:", token);
      ...
      if (result.affectedRows === 0) {
        console.warn("No se insertó el carrito.");
      }
